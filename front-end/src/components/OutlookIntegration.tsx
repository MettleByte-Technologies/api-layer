...
const OutlookIntegration = () => {
    const [accessToken, setAccessToken] = useState(
        () => sessionStorage.getItem("outlook_access_token") || ""
    );
    const [refreshToken, setRefreshToken] = useState(
        () => sessionStorage.getItem("outlook_refresh_token") || ""
    );
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            if (!authUri) throw new Error("No auth URL returned");
    ...
    const handleDisconnect = () => {
        ...
        sessionStorage.removeItem("outlook_refresh_token");
        setResponse(null);
    };
    ...
    const handleRefresh = () =>
        runApi("refresh", async () => {
            ...
            sessionStorage.setItem("outlook_access_token", data.access_token);
            if (data.refresh_token) {
                setRefreshToken(data.refresh_token);
                sessionStorage.setItem("outlook_refresh_token", data.refresh_token);
            }
            return data;
        });
    ...
    const handleCreateEvent = () =>
        runApi("create", () =>
            createEvent(eventCalendarId || undefined, accessToken, {
                subject: eventTitle,
                body: {
                    contentType: "HTML",
                    content: eventDesc || "",
                ...
                start: {
                    dateTime: formatDateToISO(eventStart),
                    timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone,
                },
                end: {
                    dateTime: formatDateToISO(eventEnd),
                    timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone,
                },
                attendees: eventAttendees.split(",").map((email) => ({
                    emailAddress: {
                        address: email.trim(),
                        name: email.trim()
                    },
        ...
        <div className="space-y-6">
            ...
            {!isConnected ? (
                <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-outlook px-4 py-3 text-sm font-medium text-outlook-foreground hover:opacity-90 transition-opacity disabled:opacity-50"
                >
                    <LogIn className="h-4 w-4" />
                    {loading === "connect" ? "Connecting..." : "Connect Outlook Account"}
                </button>
            ) : (
                <button
